...
properties = {
    ...
    projectile = { label = "Projectile", type = "object", classname = "PrefabAsset", value = nil },
    shoot_place = { label = "Shoot Place", type = "object", classname = "ComTransform", value = nil },
    ...
    attack_button = { label = "Attack Button", type = "object", classname = "ComScript", value = nil },
    home_button = { label = "Restart Button", type = "object", classname = "ComScript", value = nil },
...
function apply_accel(wish_velocity, accel)
    ...
    local accel_speed = accel * Common.frame_sec() * wish_speed
...
function apply_friction(friction)
    ...
    local drop_speed = math.max(speed, m.stop_speed) * friction * Common.frame_sec()
...
function gen_user_cmd(dx, dy)
    ...
    local angle_speed = Common.frame_time()
...
function update()
    ...
    if not m.alive and Common.real_time() - m.dead_time > 5000 then
    ...
    if m.alive then        
        --[[if m.throwing then
            local elapsed_time = Common.real_time() - m.throw_time
...
--[[
            ...
            if character_controller:is_on_ground() then
                ...
                m.velocity:set_z(m.velocity:z() - m.gravity * Common.frame_sec())
            end
            ...
            if m.velocity:length_squared() > 0.001 then
                if character_controller:move(m.velocity:mul(Common.frame_sec())) then
                    m.velocity:set_from_scalar(0)
                end
            end
--[[]]
            ...
            if skinned_mesh_renderer and m.anim_speed > 0.001 then
                ...
                if m.throwing then
                    move_delta:set_z(move_delta:z() * 4.0)
                else
                    move_delta:set_z(m.velocity:z() * Common.frame_sec())
                end
        ...
        if skinned_mesh_renderer then
            if true then
                ...
                if delta_pos:length_squared() > 0.001 then
                    m.anim_current_pos = m.anim_current_pos + delta_pos:mul(Common.frame_sec() * 5.0)
                else 
                ...
                if Math.fabs(delta_turn) > 0.001 then
                    m.anim_turn = m.anim_turn + delta_turn * Common.frame_sec() * 10.0
                else
                ...
                if Math.fabs(delta_speed) > 0.001 then
                    m.anim_speed = m.anim_speed + delta_speed * Common.frame_sec() * 10.0
                else
